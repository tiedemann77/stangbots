...
// Consulta para jss
$query = "SELECT page_namespace, page_title, page_latest, rev_timestamp, actor_name FROM page, revision, actor, comment WHERE page_content_model = 'javascript' AND page_latest = rev_id AND rev_actor = actor_id AND rev_comment_id = comment_id AND comment_text NOT LIKE '%movida automaticamente ao alterar o nome de%' AND (page_namespace != 2 OR (page_namespace = 2 AND SUBSTRING_INDEX(REPLACE(page_title, '_', ' '), '/', 1) != actor_name)) ORDER BY page_latest DESC LIMIT $limit;";
...
// Consulta para css
$query = "SELECT page_namespace, page_title, page_latest, rev_timestamp, actor_name FROM page, revision, actor, comment WHERE page_content_model = 'css' AND page_latest = rev_id AND rev_actor = actor_id AND rev_comment_id = comment_id AND comment_text NOT LIKE '%movida automaticamente ao alterar o nome de%' AND (page_namespace != 2 OR (page_namespace = 2 AND SUBSTRING_INDEX(REPLACE(page_title, '_', ' '), '/', 1) != actor_name)) ORDER BY page_latest DESC LIMIT $limit;";
